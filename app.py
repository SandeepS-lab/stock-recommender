...
INDIAN_STOCK_TICKERS = [
    ...
    'ADANIENT.NS', 'BAJFINANCE.NS', 'ZOMATO.NS', 'IRCTC.NS',
...
def get_stock_metrics(tickers, period, interval, benchmark_ticker, risk_free_rate_annual):
    ...
    try:
        ...
        else: # Fallback for other intervals
            risk_free_rate_per_period = risk_free_rate_annual / 12 if interval == "1mo" else risk_free_rate_annual / 252
    ...
    for ticker in tickers:
        try:
            ...
            volatility = stock_returns_aligned.std() * np.sqrt(len(stock_returns_aligned) / (benchmark_df.index[-1] - benchmark_df.index[0]).days * 365.25) # Annualize volatility
            ...
            if stock_returns_aligned.std() > 0 and benchmark_returns_aligned.std() > 0:
                ...
                sharpe_ratio = excess_returns_per_period / (stock_returns_aligned.std() if stock_returns_aligned.std() != 0 else np.nan)
                sharpe_ratio_annualized = sharpe_ratio * np.sqrt(len(stock_returns_aligned) / (benchmark_df.index[-1] - benchmark_df.index[0]).days * 365.25) # Annualize Sharpe
            ...
            # Using price * recent volume as a very rough proxy here - NOT RELIABLE FOR TRUE MARKET CAP
            # Or define based on a predefined list or external API fetch.
            # For now, let's keep it simple or remove if no external API is used.
...
def assign_risk_category_to_stock(beta, volatility, sharpe_ratio):
    ...
    if beta < 0.9 and volatility < 0.20 and sharpe_ratio > 0.8:
        return "Conservative"
    elif 0.9 <= beta <= 1.2 and 0.20 <= volatility <= 0.25 and sharpe_ratio > 0.6:
        return "Moderate"
    elif beta > 1.2 or volatility > 0.25: # High beta or high volatility
        return "Aggressive"
    else: # Fallback for anything in between or with less ideal Sharpe
        return "Moderate"
...
def get_stock_recommendations(risk_profile, investment_amount):
    ...
    if not df_list:
        st.warning("No valid stock data could be processed.")
        return pd.DataFrame()
    ...
    if not selected_stocks.empty:
        ...
        if risk_profile == "Conservative":
            ...
            # Higher weight to higher Sharpe, less penalty for Beta
            selected_stocks['Allocation Score'] = selected_stocks['Sharpe Ratio'] * selected_stocks['Beta'] # Can be risky, might cap beta if too high

        selected_stocks['Weight %'] = selected_stocks['Allocation Score'] / selected_stocks['Allocation Score'].sum() * 100
...
if st.button("Generate Recommendation", type="primary"):
    with st.spinner("Generating recommendations... This may take a moment as we fetch live stock data and calculate metrics."):
        ...
        if not recommended_stocks.empty:
            ...
            * **Beta:** Measures a stock's volatility in relation to the overall market (benchmark). Beta < 1 indicates lower volatility than the market; Beta > 1 indicates higher volatility.
            * **Volatility:** Measures the fluctuation of returns. Lower volatility generally means lower risk.
            * **Market Cap:** Indicates company size (Large, Mid, Small).
            """)
        else:
...
Dynamic Data Fetching with yfinance:

The get_stock_metrics function now uses yfinance to download historical data for a list of INDIAN_STOCK_TICKERS.

It calculates Sharpe Ratio, Beta, and Volatility dynamically based on historical data.

@st.cache_data(ttl=timedelta(hours=4)) is added to get_stock_metrics. This is CRUCIAL for performance. It caches the downloaded stock data and calculated metrics for 4 hours, preventing repeated downloads and calculations every time the user interacts with the app.

A BENCHMARK_TICKER (^NSEI for Nifty 50) is introduced for Beta and Sharpe Ratio calculations.

RISK_FREE_RATE_ANNUAL is added for a more accurate Sharpe Ratio calculation.

Improved error handling for yfinance downloads.

More Nuanced Risk Profiling:

Added volatility_comfort slider as a direct input for psychological risk tolerance.
...
Dynamic Stock Risk Categorization:

assign_risk_category_to_stock function dynamically categorizes each fetched stock into "Conservative," "Moderate," or "Aggressive" based on its calculated Beta, Volatility, and Sharpe Ratio. This removes the reliance on a hardcoded Risk Category column in the stock data.

Improved Portfolio Allocation:

The get_stock_recommendations function now filters stocks based on the dynamically assigned Risk Category (Stock).

It sorts stocks by Sharpe Ratio (higher is better) before selecting.

Introduced num_stocks_to_recommend to control portfolio diversification based on risk profile (e.g., more stocks for conservative, fewer for aggressive concentration).

The Allocation Score calculation is made more strategic:

Conservative: Prioritizes high Sharpe and low Beta.

Moderate: Balances Sharpe and Beta.

Aggressive: Leans towards higher Sharpe, potentially even higher Beta (though careful with pure Beta weighting for aggressive as it can lead to very concentrated volatile portfolios).

Added a fallback mechanism: if no direct-match stocks are found for a risk profile, it tries to broaden the search (e.g., Conservative also considers Moderate stocks).

Enhanced Streamlit UI/UX:

Used st.expander to organize input fields, making the UI cleaner.

Added help text to sliders and inputs for better user guidance.

Added st.spinner during data fetching to inform the user about processing time.

Included a Pie Chart using matplotlib to visualize portfolio allocation, which is much more intuitive for fund managers.

Added a section "Understanding the Metrics" to educate the user on Sharpe Ratio, Beta, and Volatility.

Added a prominent Disclaimer.

Further Potential Enhancements (Beyond this current improvement):

Advanced Portfolio Optimization: Integrate PyPortfolioOpt for Mean-Variance Optimization, Black-Litterman, or other sophisticated allocation strategies.

Fundamental Data API: To get accurate Market Cap, P/E, Debt/Equity, etc., for more robust filtering and analysis (e.g., from Alpha Vantage, although free tiers have limitations).

Sentiment Analysis: Integrate news or social media sentiment for stocks.

Economic Indicators: Allow the model to factor in current economic conditions.

User Authentication/Profiles: If this were a real application for multiple fund managers/clients.

Backtesting Framework: A more elaborate backtesting module to show historical performance of similar strategies.

Predictive Model (ML/DL): Instead of just rule-based allocation based on metrics, you could train an actual ML model (e.g., XGBoost, LSTM) to predict future stock performance, and then use those predictions in your portfolio optimization.

This improved version is much more robust and intelligent than the initial one, especially by incorporating live data and dynamic metric calculation. Remember to install yfinance and matplotlib if you haven't already:

pip install yfinance streamlit matplotlib pandas numpy
...
kindlly make the requisite changes and share the .py file


Since I cannot directly create or upload files, I will provide you with the complete and updated Python code. You can then copy this code and save it as enhanced_app.py on your system.

Before you run the code, ensure you have the necessary libraries installed:

Bash

pip install streamlit pandas numpy yfinance matplotlib
Here's the complete, improved enhanced_app.py file content:
